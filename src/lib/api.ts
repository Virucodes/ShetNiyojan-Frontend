...
export const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://shetniyojan-backend.onrender.com/api';
...
export const otp = {
  send: async (mobile: string) => {
    const response = await api.post('/otp/send', { mobile });
    return response.data;
  },
  verify: async (mobile: string, code: string) => {
